...
<header class="masthead">
  ...
  <h1>优先清旧构建，保留最新一份和下载缓存</h1>
  <p>扫描时核对源码和构建配置，每项目保留最新一份，将其余可重建的完整编译目录列为建议删除。Cargo 下载缓存、依赖环境和插件包保留，避免反复下载。下面给出拟删路径、保留路径和重建方式。</p>
</header>
<div id="app"></div>
</main>
<script>
...
function drawCandidateGauge(svg){
  ...
  svgText(svg,{x:cx,y:cy+14,'text-anchor':'middle','font-size':13,'font-weight':700,fill:INK},'建议清理');
...
function renderHero(){
  ...
  return '<section class="hero cleanup-hero"><div class="hero-main"><div class="hero-label">建议删除的旧构建产物</div><div class="hero-number"><span class="accent">'+fmtBytes(s.old_build_bytes)+'</span></div><div class="hero-meta">'+(s.old_build_count||0)+' 项 · 只包含 Codex 生成或管理的文件 · 生成报告不会改变文件</div></div><div class="hero-side"><strong>已检查 '+(scope.readable_user_count||0)+' 个用户</strong><div class="rule"></div><p>'+esc(p.name||'当前平台')+' · '+esc(p.arch||'')+' · 当前账号 '+esc(p.current_user||'未知')+'</p><p>找到了 '+(scope.codex_root_count||0)+' 个 Codex 文件夹；得到你的同意后才会移动或删除。</p></div></section><div class="report-strip"><div class="report-stat green"><b>'+fmtBytes(s.regenerable_bytes)+'</b><span>建议清理合计 · '+(s.regenerable_count||0)+' 项</span></div><div class="report-stat orange"><b>'+fmtBytes(s.manual_bytes)+'</b><span>未确认与用户数据 · '+(s.manual_count||0)+' 项</span></div><div class="report-stat"><b>'+fmtBytes(s.codex_bytes)+'</b><span>Codex 文件总量</span></div><div class="report-stat"><b>'+fmtBytes(s.protected_bytes)+'</b><span>建议保留的文件</span></div></div>';
}
function renderCharts(){
  return '<section class="chart-grid">'+renderCard('建议清理的文件有多少？','F11 · 只计算已经找到的 Codex 文件夹和待处理文件','<svg id="candidate-gauge" class="chart-square" data-chart="candidate-gauge" aria-label="建议清理的文件占 Codex 文件比例"></svg>','SCOPE / RATIO')+renderCard('哪个用户占用最多？','每个圆环代表一个找到 Codex 文件的用户','<svg id="user-lens" class="chart-lens" data-chart="user-lens" aria-label="各用户的 Codex 文件占用"></svg>','USERS / LENS')+renderCard('这些文件是什么时候留下的？','F2 · 按文件夹最近修改时间统计','<svg id="codex-timeline" class="chart-history" data-chart="codex-timeline" aria-label="Codex 文件时间线"></svg>','AGE / TIMELINE')+renderCard('具体是哪些文件夹？','G10 · 条形只代表建议清理的部分，不代表整个工作区','<svg id="codex-paths" class="chart-wide" data-chart="codex-paths" aria-label="建议删除的旧构建产物夹"></svg>','PATHS / LEDGER')+'</section>';
}
function renderScope(){
  ...
  return '<section class="section"><div class="section-head"><h2>检查了哪些用户和文件夹？</h2><div class="source">DISCOVERY / PROVENANCE</div></div><p class="scope-note">逐个检查每个用户实际存在的 <code>.codex</code> 文件夹、Documents/Codex、Documents/ChatGPT 工作区探测范围和 Codex 应用数据。不会把整个用户文件夹、Documents 或 C:\\Users 当成清理目标。</p><div class="table-wrap"><table class="table scope-table"><thead><tr><th>用户 / 文件夹</th><th>平台</th><th>检查结果</th><th>找到的 Codex 文件夹</th><th>Codex 占用</th><th>建议清理</th></tr></thead><tbody>'+rows+'</tbody></table></div></section>';
}
...
function renderItem(it,tier,maxSize){
  const size=bytes(it.size_bytes),pct=Math.min(100,size/Math.max(maxSize,1)*100),tag=tier==='regenerable'?'<span class="ledger-tag green">建议清理</span>':tier==='manual'?'<span class="ledger-tag orange">先看一下</span>':'<span class="ledger-tag">建议保留</span>';
  ...
  const meta='<dl class="meta-grid"><dt>文件位置</dt><dd class="path">'+esc(it.path)+'</dd><dt>为什么算作 Codex 文件</dt><dd>'+esc(it.provenance)+'</dd><dt>为什么现在列出来</dt><dd>'+esc(it.rebuild_basis||it.reason)+'</dd><dt>是否正在使用</dt><dd>'+esc(it.activity||'没有找到相关信息')+'</dd><dt>项目状态</dt><dd>'+esc(it.git_state||'不适用')+'</dd><dt>保留哪一份</dt><dd>'+esc(it.retained_path||'不适用')+'</dd><dt>如何重建</dt><dd>'+esc(it.rebuild_command||'见恢复说明')+'</dd><dt>流量与时间成本</dt><dd>'+esc(it.network_cost||'未核实')+'；'+esc(it.rebuild_time||'未测量')+'</dd><dt>怎么恢复</dt><dd>'+esc(it.recovery||'请先确认')+'</dd><dt>建议怎么处理</dt><dd>'+esc(it.recommendation||'保留')+'</dd></dl>';
  const source=it.source_root?'<p><span class="label">源码保留在</span> '+esc(it.source_root)+' · '+esc(it.source_revision||'未提交仓库')+'</p>':'';
  const extra=source+(it.file_count!=null?'<p><span class="label">归档文件数量</span> '+esc(it.file_count)+(it.file_count_note?' · '+esc(it.file_count_note):'')+'</p>':'');
...
function renderDecision(){
  const green=DATA.green||[],old=green.filter(it=>it.review_priority==='delete-old-build'),secondary=green.filter(it=>it.review_priority!=='delete-old-build');
  return '<section class="section"><div class="section-head"><h2>建议清理旧构建 '+fmtBytes((DATA.summary||{}).old_build_bytes)+'</h2></div><div class="ledger-summary"><div><h2>源码保留，每项目留最新一份构建</h2><p>拟删目录已核对源码入口、构建配置和产物结构。不同分支和编译配置不重复占用保留名额；需要时重新编译。这里只做了静态核查，没有实际运行构建。</p></div><div><p>Cargo registry/git、工具链和依赖包默认保留。正在使用或唯一的产物保留。</p><p>移入同盘回收站通常不会立即增加磁盘可用空间。</p></div></div>'+renderTier('建议删除 · 可从源码重建的旧目录','OLD BUILDS','展开查看源码、重建命令和最新保留路径。',old,'regenerable')+renderTier('保留 · 最新构建、依赖和核心数据','KEEP','每项目最新一份及正在使用的目录；源码和下载缓存保留。',DATA.red||[],'protected')+renderTier('次选 · 缓存和临时文件','SECONDARY','收益较小，仍需确认实际恢复成本。',secondary,'regenerable')+renderTier('未确认与用户数据','UNRESOLVED','列明具体问题，不计入建议清理量。',DATA.yellow||[],'manual')+'</section>';
}
