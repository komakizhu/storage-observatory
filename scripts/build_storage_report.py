...
function renderDenied(){const a=DATA.denied||[];return a.length?'<section class="section denied"><b>权限不足，可能遗漏：</b><br>'+a.map(esc).join('<br>')+'</section>':''}
const tierScaleMax=items=>Math.max(...(items||[]).map(it=>n(it.size_estimate||it.size)),1);
const app=document.getElementById('app');
const renderBelowFold=()=>renderUsers()+renderTop5()+'<section class="section"><div class="section-head"><h2>清理决策清单</h2></div>'+renderTier('可自动清理 · 纯缓存',DATA.green,'green',tierScaleMax(DATA.green))+renderTier('需人工判断 · 用户数据',DATA.yellow,'yellow',tierScaleMax(DATA.yellow))+renderTier('谨慎清理 · 正规卸载',DATA.red,'red',tierScaleMax(DATA.red))+'</section>';
