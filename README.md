...
一个面向 macOS 的、手动触发的存储空间观察与可视化报告工具。它每次被启用时记录一枚只读快照，把当前磁盘状态、历史变化、占用来源和清理决策放进同一份可视化网页报告。
...
报告使用十进制容量单位展示数据：1 TB = 1000 GB。同时保留物理磁盘、APFS 容器、文件系统和目录扫描之间的口径差异，避免把厂商标称的 1 TB、macOS 显示的容量以及某个目录扫描结果混为一谈。
...
- 硬盘空间突然下降，想知道增长来自哪里；
- macOS 的容量数字与 du、Finder 或磁盘工具不一致，需要区分 APFS 容器、系统卷、用户目录和未展开项目；
...
2. **原存储分析器**
   ...
   - 原项目链接：目前未找到可验证的公开 GitHub/GitLab 上游地址；本机来源是独立安装的 Codex skill

   它主要提供存储分析的语义和安全基础：macOS 只读扫描、总容量与目录口径区分、占用分类、清理决策清单、缓存与用户数据的判断方式，以及“移到废纸篓优先、直接删除需谨慎”的安全边界。存储观察站在此基础上增加了手动快照历史、差异报告、全盘根目录对账和可视化观察流程。
...
扫描阶段只读，使用 macOS 自带的 df、du、diskutil、stat、ls 等工具读取容量和目录信息，不会在扫描时删除、移动、改权限或清空废纸篓。
...
**1.0.0-beta1**：功能和视觉方向基本定稿，适合在本机 macOS 工作流中使用。后续如果继续开发，建议把新的变化记录为 beta2，而不是悄悄改变 1.0.0-beta1 的既有行为。
